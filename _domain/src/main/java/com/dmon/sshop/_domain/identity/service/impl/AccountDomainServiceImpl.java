...
public class AccountDomainServiceImpl implements IAccountDomainService {
    ...
    public AccountRes createOne(AccountReq.Create accountDto) {
        ...
        accountCreated.setRoles(new HashSet<>(Collections.singletonList(Account.RoleEnum.USER.name())));
    ...
    public Account preparePreCreate(AccountReq.Create accountDto, Account.RoleEnum roleEnum) {
        ...
        if (Objects.nonNull(accountFound)) {
            ...
            if (accountFound.getRoles().contains(Account.RoleEnum.USER.name()))
                throw new AppException(ErrorCode.SECURITY__USER_NOT_REGISTER_SELLER);
            if (accountFound.getRoles().contains(Account.RoleEnum.SELLER.name()))
                throw new AppException(ErrorCode.SECURITY__SELLER_NOT_REGISTER_SELLER);
            if (accountFound.getRoles().contains((Account.RoleEnum.ADMIN.name())))
