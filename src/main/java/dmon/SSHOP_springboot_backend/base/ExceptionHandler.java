...
public class ExceptionHandler {
    ...
    ResponseEntity<ApiRes<Object>> handleException(HttpRequestMethodNotSupportedException exception){
        ExceptionCode exceptionCode = ExceptionCode.SYSTEM__METHOD_NOT_SUPPORTED;
        return ResponseEntity
                .status(exceptionCode.getStatus())
                .body(ApiRes.builder()
                        .code(exceptionCode.getCode())
                        .error(String.format(exceptionCode.getMessage(), exception.getMethod()))
                        .build()
                );
    }
    ...
    ResponseEntity<ApiRes<Object>> handleException(NoResourceFoundException exception){
        ExceptionCode exceptionCode = ExceptionCode.SYSTEM__ROUTE_NOT_SUPPORTED;
        return ResponseEntity
                .status(exceptionCode.getStatus())
                .body(ApiRes.builder()
                        .code(exceptionCode.getCode())
                        .error(String.format(exceptionCode.getMessage(), exception.getResourcePath()))
                        .build()
                );
    }
}
